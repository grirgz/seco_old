...
~mk_sequencer = { 
	...
	make_ve_handlers: { arg self;
		...
		size_y = min(4, self.model.varset_dict[cps].size);
		size_x = min(8, self.model.var_dict[cvs].size);
		...
		size_y.do { arg rowidx;
			...
			size_x.do { arg idx;
				...
				self.kb_handler[ [0, ~kbpad8x4[rowidx][idx]] ] = { 
					...
					var cv; 
					var ev;
					[cps, cidx, self.model.var_dict].debug("cur_cps cidx, varset_dict");
					cur_varset = self.model.varset_dict[cps][cidx];
					cur_varset.debug("cur_varset");
	...
	make_handlers: { arg self;
		...
		self.kb_handler[ [0, ~kbfx[11]] ].debug("bla1");
